...
<div class="col-md-12">
    <div class="table-responsive">
        <table class='table table-bordered table-striped table-responsive part_{{$manufacturingPart}}' id="part_{{$manufacturingPart}}">
            <thead>
            <tr>
                <th><b>Part No</b></th>
                <th><b>Order</b></th>
                <th><b>Ref Route Card</b></th>
                <th><b>Cover No</b></th>
                <th><b>Cover Qty</b></th>
                <th><b>Route Card Available Quantity</b></th>
                <th><b>DC Quantity</b></th>
                <th><b>Balance</b></th>
            </tr>
            </thead>
            <tbody >
                ...
                <tr class="order_{{$packingStrickerDetail->id}}">
                    <input type="hidden" name="id[]" value="{{$packingStrickerDetail->id}}">
                    <td width="250px"><select name="route_part_id[]" class="form-control bg-light route_part_id" id="route_part_id"><option value="{{$packingStrickerDetail->partmaster->id}}">{{$packingStrickerDetail->partmaster->child_part_no}}</option></select></td>
                    <td><input type="number" name="order_no[]" @readonly(true) class="form-control bg-light order_no"  id="order_no" value="{{$packingStrickerDetail->cover_order_id}}"></td>
                    <td width="250px"><select name="route_card_id[]" class="form-control bg-light route_card_id" id="route_card_id"><option value="{{$packingStrickerDetail->rcmaster->id}}">{{$packingStrickerDetail->rcmaster->rc_id}}</option></select></td>
                    <td><input type="number" name="cover_order_id[]"  class="form-control bg-light cover_order_id"  id="cover_order_id" value="{{$packingStrickerDetail->cover_order_id}}"></td>
                    <td><input type="number" name="cover_qty[]"  class="form-control bg-light cover_qty"  id="cover_qty" value="{{$packingStrickerDetail->ok_packed_qty}}"></td>
                    <td><input type="number" name="available_quantity[]" @readonly(true) class="form-control bg-light available_quantity"  id="available_quantity" value="{{$packingStrickerDetail->avl_qty}}"></td>
                    <td><input type="number" name="issue_quantity[]"  class="form-control bg-light issue_quantity" id="issue_quantity" min="0" max="{{$packingStrickerDetail->avl_qty}}" value="{{$packingStrickerDetail->avl_qty}}" ></td>
                    <td><input type="number" name="balance[]"  class="form-control bg-light balance" id="balance" min="0" max="{{$packingStrickerDetail->avl_qty}}" value="{{($packingStrickerDetail->avl_qty)-($packingStrickerDetail->avl_qty)}}"></td>
                    </tr>
                @empty
                <tr>
...
<div class="row mb-3 d-flex justify-content-end clearfix" style="background-color: aliceblue">
    @for ($i = 0; $i < 1; $i++)
        @if ($i==0)
            @if (($packingStrickerDetails[$i]->avl_qty)>($dc_quantity))
                @php
                    $grand_total=$dc_quantity;
                    $total_diff=($dc_quantity)-$grand_total;
                @endphp
            @elseif(($packingStrickerDetails[$i]->avl_qty)<=($dc_quantity))
                @php
                    $grand_total=$packingStrickerDetails[$i]->avl_qty;
                    $total_diff=($dc_quantity)-$grand_total;
                @endphp
            @endif
        @else
    ...
    <div class="col-2"><input type="number" name="grand_total[{{$manufacturingPart}}]" class="form-control bg-light" id="grand_total_{{$manufacturingPart}}" value="{{$dc_quantity}}"  readonly>
    </div>

    <div class="col-2"><h6>Balance:</h6></div>
    <div class="col-2"><input type="number" name="total_diff[{{$manufacturingPart}}]" class="form-control bg-light" id="total_diff_{{$manufacturingPart}}" value="{{($dc_quantity)-($dc_quantity)}}"  readonly></div>
    @endfor
