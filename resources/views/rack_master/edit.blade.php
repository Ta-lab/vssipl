...
<div class="row d-flex justify-content-center">
    <div class="col-8 ">
        ...
        <div class="row col-md-3"id="res"></div>
            ...
            <div class="card-body">
            <form action="{{route('rackmaster.update',$rackmaster->id)}}" id="rack-master_formdata" method="POST">
                @csrf
                @method('PUT')

                <input type="hidden" name="id" value="{{$rackmaster->id}}">
                <div class="row justify-content-center">
                        ...
                        <div class="form-group">
                            ...
                            <select name="stocking_id" id="stocking_id" class="form-control @error('stocking_id') is-invalid @enderror">
                                ...
                                @forelse ($rackstockmasters as $rackstockmaster)
                                    <option value="{{$rackstockmaster->id}}" @if ($rackstockmaster->id==$rackmaster->stocking_id) selected @endif>{{$rackstockmaster->name}}</option>
                                @empty
                                @endforelse
                            </select>
                            @error('stocking_id')
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="text" name="rack_name" class="form-control @error('rack_name') is-invalid @enderror" value="{{$rackmaster->rack_name}}" >
                            @error('rack_name')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                        ...
                        <div class="form-group">
                            ...
                            <select name="raw_material_category_id" id="raw_material_category_id" class="form-control @error('raw_material_category_id') is-invalid @enderror">
                                ...
                                @forelse ($categories as $category)
                                    <option value="{{$category->id}}" @if ($category->id==$rackmaster->raw_material_category_id) selected @endif>{{$category->name}}</option>
                                @empty
                                @endforelse
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="raw_material_id" id="raw_material_id" class="form-control @error('raw_material_id') is-invalid @enderror">
                                ...
                                @forelse ($rm_datas as $rm_data)
                                    <option value="{{$rm_data->id}}" @if ($rm_data->id==$rackmaster->raw_material_id) selected @endif>{{$rm_data->name}}</option>
                                @empty
                                @endforelse
                            </select>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control">
                                <option value="1" @if($rackmaster->status==1) selected @endif >Active</option>
                                <option value="0" @if($rackmaster->status==0) selected @endif>Inactive</option>
                            </select>
                            @error('name')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
