...
@endpush
@section('content')
<form action="{{route('ptsreworkrevert.store')}}" id="pts_receive_formdata" method="POST">
    @csrf
    @method('POST')
...
<div class="row d-flex justify-content-center">
    ...
    @endif
        ...
        <div class="card">
            <div class="card-header d-flex" style="justify-content:space-between"><span> <b>PTS Production Material Rework Revert Register</b></span><a class="btn btn-sm btn-primary" href="{{route('ptsreworkrevertlist')}}">Rework Revert List</a>
            </div>
            <div class="card-body">
                        <div class="row d-flex justify-content-center">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="part_id" id="part_id" class="form-control @error('part_id') is-invalid @enderror">
                                        ...
                                        @foreach ($rework_rcdatas as $rework_rcdata)
                                            <option value="{{$rework_rcdata->partmaster->id}}" >{{$rework_rcdata->partmaster->child_part_no}}</option>
                                        @endforeach
                                    </select>
                                    @error('part_id')
                        ...
                        <div class="row d-flex justify-content-center">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="rework_revert_qty">Issue Quantity In Numbers*</label>
                                    <input type="number" name="rework_revert_qty" id="rework_revert_qty" required min="0" class="form-control @error('rework_revert_qty') is-invalid @enderror">
                                    @error('rework_revert_qty')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                    @enderror
...
$("#part_id").select2({
    ...
    $('#part_id').change(function (e) {
        ...
        if (part_id!='') {
            $.ajax({
                type: "POST",
                url: "{{ route('ptsrcreworkrevertfetchdata') }}",
                data:{
                    "_token": "{{ csrf_token()}}",
                    "part_id":part_id,
                },
    ...
    $('#rc_no').change(function (e) {
        ...
        if (rc_no!='') {
            $.ajax({
            type: "POST",
            url: "{{ route('ptsreworkrevertpartreceivefetchdata') }}",
            data:{
                "_token": "{{ csrf_token()}}",
                "rc_no":rc_no,
                "part_id":part_id,
            ...
            success: function (response) {
                ...
                //  if(response.success){
                        ...
                        //  $('#part_id').html(response.part);
                         ...
                         $('#avl_qty').val(response.rework_avl_qty);
                         $('#rework_revert_qty').attr('max', response.rework_avl_qty);
                         $('#rework_revert_qty').attr('min', 0);
    ...
    $('#rework_revert_qty').change(function (e) {
        e.preventDefault();
        var avl_qty=$('#avl_qty').val();
        var rework_revert_qty=$(this).val();
        var rc_no=$('#rc_no').val();
        var part_id=$('#part_id').val();
        if (avl_qty!=''&&rework_revert_qty!='') {
                var diff=avl_qty-rework_revert_qty;
                // alert(diff);
                if(diff<0){
                ...
                $('#inlineRadio1').hide();
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('ptsreworkstickerfetchdata') }}",
                        data:{
                            "_token": "{{ csrf_token()}}",
                            "rc_no":rc_no,
                            "part_id":part_id,
                            "avl_qty":avl_qty,
                            "rework_revert_qty":rework_revert_qty,
                        },
                        success: function (response) {
                            $('#data1').html(response.html);
                        }
                    });
                }
        }else{
